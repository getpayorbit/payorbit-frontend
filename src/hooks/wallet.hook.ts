...
export function useCompanyWallets(companyId?: string) {
	...
	useEffect(() => {
		if (query.data?.data) {
			setWallets(query.data.data.data);
		}
...
export function useCompanyWalletDetails(walletId?: string, companyId?: string) {
	...
	useEffect(() => {
		if (query.data?.data) {
			setSelectedWallet(query.data.data.data);
			upsertWallet(query.data.data.data);
		}
...
export function useCreateCompanyWallet(companyId?: string) {
	...
	return useMutation({
		...
		onSuccess: (response) => {
			upsertWallet(response.data.data);
			...
			queryClient.setQueryData(
				["wallets", "detail", resolvedCompanyId, response.data.data.id],
				response,
			);
		},
	});
...
export function useMyWalletDetails(enabled = true) {
	...
	useEffect(() => {
		if (query.data?.data) {
			setMyWallet(query.data.data.data);
		}
...
export function useMyWalletPrivateKey(enabled = false) {
	...
	useEffect(() => {
		if (query.data?.data) {
			setMyPrivateKey(extractWalletPrivateKey(query.data.data.data));
		}
