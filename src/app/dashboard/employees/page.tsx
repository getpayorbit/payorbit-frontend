...
export default function EmployeesPage() {
	...
	const walletDetailsQuery = useEmployeeWalletDetails(
		detailsEmployeeId ?? undefined,
	);
	...
	const employeesWithWallets = employees.filter(
	...
	const handleWalletSubmit = async () => {
		...
		if (
			...
			!STELLAR_PUBLIC_KEY_PATTERN.test(trimmedWalletAddress)
		) {
		...
		try {
			await linkEmployeeWallet({
				...
				payload: trimmedWalletAddress
					? { stellar_address: trimmedWalletAddress }
					: {},
			});
			toast.success(
				trimmedWalletAddress
				...
				<FadeUp delay={40}>
					<Alert className="border-amber-200 bg-amber-50 text-amber-900">
						...
						<AlertDescription className="text-amber-800">
							The page remains usable, but one or more employee endpoints are
					...
					<FadeUp delay={180}>
						<Card className="p-5">
							...
							<div className="mt-4 flex flex-wrap gap-2">
								{Object.entries(employeeStats.by_employment_type).length ===
								0 ? (
									<p className="text-sm text-muted-foreground">
								...
								<tbody>
									{filteredEmployees.map((employee) => (
										...
										>
											...
											<td className="px-4 py-3">
												<StatusBadge status={(employee.status ?? "inactive").toLowerCase()} />
											</td>
											<td className="px-4 py-3">
